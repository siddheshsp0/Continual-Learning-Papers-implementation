...
def evaluate(model: DualPromptModel, loader: DataLoader, task_id: int, device: torch.device) -> float:
    model.eval()
    correct = total = 0
    with torch.no_grad():
        for x, y in loader:
...
def train_task(model: DualPromptModel, loader: DataLoader, task_id: int, cfg: dict, device: torch.device):
...
def main():
    ...
    for task_id in range(cfg['num_tasks']):
        ...
        train_task(model, loader, task_id, cfg, device)
        ...
        for prev_t in range(task_id + 1):
            ...
            acc = evaluate(model, test_loader, prev_t, device)
            accs.append(acc)
