...
def evaluate(model: CODAPromptModel, loader: DataLoader, device: torch.device) -> float:
    model.eval()
    correct = total = 0
    with torch.no_grad():
        for x, y in loader:
...
def train_task(model: CODAPromptModel, loader: DataLoader, cfg: dict, device: torch.device):
    params = [p for p in model.parameters() if p.requires_grad]
    opt = torch.optim.Adam(params, lr=cfg['lr'])
    ce = nn.CrossEntropyLoss()
    lam = cfg['ortho_loss_weight']
    model.train()
    for epoch in range(cfg['epochs_per_task']):
        ...
        for x, y in loader:
            x, y = x.to(device), y.to(device)
            logits = model(x)
            loss = ce(logits, y) + lam * model.ortho_loss()
            opt.zero_grad()
            loss.backward()
            opt.step()
...
def main():
    ...
    model = CODAPromptModel(cfg, num_classes=cfg['classes_per_task']).to(device)
    ...
    for task_id in range(cfg['num_tasks']):
        ...
        loader = DataLoader(train_set, batch_size=cfg['batch_size'], shuffle=True, num_workers=4, pin_memory=True)
        train_task(model, loader, cfg, device)
