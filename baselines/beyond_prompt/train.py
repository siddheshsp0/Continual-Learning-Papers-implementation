...
def evaluate(model: ContinualAdapterModel, loader: DataLoader, device: torch.device) -> float:
    model.eval()
    correct = total = 0
    with torch.no_grad():
        for x, y in loader:
...
def train_task(model: ContinualAdapterModel, loader: DataLoader, cfg: dict, device: torch.device):
...
def main():
    ...
    model = ContinualAdapterModel(cfg, num_classes=cfg['classes_per_task']).to(device)
